...
// Still waiting check (3h+)
const stillWaitingVariants = [
  (n) => `Hola ${n}! Ya llevas un buen rato en la fila de Chuí. Queres seguir esperando?\n\nResponde:\n1 - Si, sigo esperando\n2 - No, cancelar`,
  (n) => `${n}, vemos que llevas bastante esperando en Chuí. Seguimos contando con vos?\n\nResponde:\n1 - Si, me quedo\n2 - Cancelar`,
  (n) => `Hola ${n}! Sabemos que la espera es larga. Queres que te mantengamos en la fila de Chuí?\n\nResponde:\n1 - Si, espero\n2 - No, cancelo`,
];
...
export function msgStillWaiting({ guestName }) {
  return pick(stillWaitingVariants)(guestName);
}
...
const textToJoinVariants = [
  ...
  (n, p) => `Hola ${n}! Ya estas en la fila de Chui, posicion #${p}. Te avisamos apenas haya mesa.\n\nPodes escribir "cuanto falta" o "menu" cuando quieras.`,
  (n, p) => `Listo ${n}! Estas #${p} en la fila de Chui. Te mandamos mensaje cuando este tu mesa.\n\nEscribi "cuanto falta" para ver tu estado.`,
];
...
const arrivingConfirmVariants = [
  (n, m) => `Perfecto ${n}, te esperamos en ~${m}min! Loyola 1250.`,
  (n, m) => `Genial ${n}! Te guardamos la mesa. Nos vemos en ${m}min.`,
  (n, m) => `Dale ${n}, te esperamos! Loyola 1250, Villa Crespo.`,
];

const runningLateVariants = [
  (n, m) => `No hay problema ${n}, te guardamos la mesa ${m}min mas. Veni tranqui!`,
  (n, m) => `Tranqui ${n}! Te esperamos ${m}min extra. Loyola 1250.`,
  (n, m) => `Dale ${n}, no te preocupes. Te guardamos la mesa ${m} minutitos mas.`,
];

const menuLinkVariants = [
  (n) => `${n}, aca tenes nuestra carta: https://mesa-xi.vercel.app?menu=1\nTe esperamos!`,
  (n) => `Hola ${n}! Mira nuestro menu aca: https://mesa-xi.vercel.app?menu=1`,
  (n) => `${n}, podes ver la carta en: https://mesa-xi.vercel.app?menu=1\nBuen provecho!`,
];

const thanksVariants = [
  (n) => `Buen provecho! Gracias por elegir Chui, ${n}!`,
  (n) => `Gracias a vos ${n}! Esperamos verte pronto en Chui.`,
